...
(define (parse [s : S-Exp]) : ExpI
  (cond
   ...
   [(s-exp-match? `null s) (nullI)]                                     ;;condition to parse null Part4
   ...
   [(s-exp-match? `{* ANY ANY} s)
    (multI (parse (second (s-exp->list s)) )
           (parse (third (s-exp->list s)) ))]
   [(s-exp-match? `{cast SYMBOL ANY} s)                                 ;;condition to parse cast instruction Part2
    (castI (s-exp->symbol (second (s-exp->list s))) 
           (parse (third (s-exp->list s)) ))]
   [(s-exp-match? `{if0 ANY ANY ANY} s)                                  ;;condition to parse if0 instruction Part3
   ...
   [(s-exp-match? `{set ANY SYMBOL ANY} s)                                ;;condition to parse set instruction Part5
...
(module+ test
  (test (parse `0)
        (numI 0)) 
  (test (parse `null )                                                           ;;test case for parsing null Part4
        (nullI))                         
  (test (parse `arg )
        (argI))
  (test (parse `this )
  ...
  (test (parse `{* 1 2} )
        (multI (numI 1) (numI 2)))  
;; ----------------------------------------  
  (test (parse `{cast Posn {new Posn 1 2}})                              ;;test case for parsing cast Part2
        (castI 'Posn (newI 'Posn (list (numI 1) (numI 2)))))
  (test (parse `{if0 1 2 3})                                             ;;test case for parsing if0 Part3
        (if0I (numI 1) (numI 2) (numI 3)))    
  (test (parse `{get 1 x})
        (getI (numI 1) 'x))
  (test (parse `{set {new Posn 1 2} x 5} )                               ;;test case for parsing set Part5
        (setI (newI 'Posn (list (numI 1) (numI 2))) 'x (numI 5)))
...
(define (interp-prog [classes : (Listof S-Exp)] [a : S-Exp]) : S-Exp
  (let ([v (interp-i (parse a)
    ...
    (type-case Value v
      ...
      [(nullV) `null]
      [(objV class-name super-name field-vals) `object])))

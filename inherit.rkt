...
(define-type ExpI
  ...
  (multI [lhs : ExpI]
         [rhs : ExpI])
  (castI [lhs : Symbol]                               ;;ExpI variant for cast operator Part2
         [rhs : ExpI])
  (if0I [chk : ExpI]                                  ;;ExpI variant for if0 operator Part3
       [t : ExpI]
       [f : ExpI])
  (nullI)                                             ;;ExpI variant for null expressions Part4
  (argI)
  (thisI)
  (newI [class-name : Symbol]
        [field-expr : (Listof ExpI)])
  (getI [obj-expr : ExpI]
        [field-name : Symbol])
  (setI [obj-expr : ExpI]                             ;;ExpI variant for set operator Part5
        [field-name : Symbol]
        [field-exp : ExpI])
  (sendI [obj-expr : ExpI]
         [method-name : Symbol]
...
(define (exp-i->c [a : ExpI] [super-name : Symbol]) : Exp
  (local [(define (recur expr)
    ...
    (type-case ExpI a
      ...
      [(castI l r) (castE l (recur r))]                                ;;exp-i->c case for castI to castE Part2
      [(if0I c t e) (if0E (recur c) (recur t) (recur e))]              ;;exp-i->c case for if0I to if0E Part3
      [(nullI)(nullE)]                                                 ;;exp-i->c case for nullI to nullE Part4
      [(argI) (argE)]
      [(thisI) (thisE)]
      ...
      [(setI obj-expr field-name field-exp)                             ;;exp-i->c case for setI to setE Part5
       (setE (recur obj-expr) field-name (recur field-exp))]
...
(module+ test
  ...
  (test (exp-i->c (castI 'Posn (newI 'Posn (list (numI 1) (numI 2)))) 'Object)          ;;test case for exp-i->c case for castI to castE Part2
        (castE 'Posn (newE 'Posn (list (numE 1) (numE 2)))))
  (test (exp-i->c (if0I (numI 1) (numI 2) (numI 3)) 'Object)                            ;;test case for exp-i->c case for if0I to if0E Part3
        (if0E (numE 1) (numE 2) (numE 3)))
  (test (exp-i->c (nullI) 'Object)                                                      ;;test case for exp-i->c case for nullI to nullE Part4
        (nullE))  
  ...
  (test (exp-i->c (getI (numI 1) 'x) 'Object)
        (getE (numE 1) 'x))
  (test (exp-i->c (setI (newI 'Object (list (numI 1))) 'x (numI 10)) 'Object)            ;;test case for exp-i->c case for setI to setE Part5
        (setE (newE 'Object (list (numE 1))) 'x (numE 10)))  
  ...
  (type-case Class (find classes-not-flat name)
    [(classC field-names super-name methods)
     (type-case Class (flatten-super name classes-not-flat i-classes)
       [(classC super-field-names super-super-name super-methods)
        (classC
  ...
  (local [(define a (exp-i->c i-a 'Object))
          ...
          (define classes
            (map (lambda (c)
                 ...
                 classes-not-flat))]
    (interp a classes (objV 'Object 'Object empty) (numV 0))))
